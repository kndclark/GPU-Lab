...
## The overnight run, corrected 2026-09-16 (per-file isolation)

The first overnight run (above) mixed real failures with cascade collateral.
Re-run after the per-file fix landed (`7338f03`); both nodes finished all 195
files across the six directories and restored serving from the EXIT trap. This
section supersedes the raw counts above -- keep those for the cascade story,
not as a scoreboard.

| directory | sm_86 (3090) | sm_120 (5090) | cascaded files |
|---|---|---|---|
| quantization | 5899: 2307P 197F 17E 3378S | 7223: 3013P 1033F 271E 2906S | 1 (`test_block_fp8`, contained) |
| moe | 8004: 6044P 47F 1913S | 8811: 7148P 97F 22E 1544S | 0 |
| attention | 8272: 3426P 2757F 2089S | 8280: 5980P 363F 1937S | 0 |
| core | 3308: 3181P 71F 56S | 3308: 3208P 53F 47S | 0 |
| ir | 1627: 1443P 184S | 1627: 1443P 184S | 0 |
| mamba | 929: 895P 8F 26S | 929: 895P 8F 26S | 0 |

Per-file isolation worked: 195 processes instead of 6, and only one file
(`test_block_fp8`) still carries an internal cascade, contained to itself.
sm_120's moe failures dropped from 1939 to 97+22 and quantization's real
failure count is now visible rather than buried under 6777 collateral results.

**A caveat from the first run dissolved rather than resolved.** The 87 sm_86
attention OOM failures reported yesterday are **gone** with per-file isolation
-- zero OOM failures on sm_86 in this run. They were CUDA allocator
fragmentation from running thousands of GEMM-heavy tests in one long-lived
pytest process, not a capacity limit on the card. This is itself a finding
about the harness, not about the hardware: never trust a memory-pressure result
from a multi-file run.

### The fifth finding, and the cleanest one yet: the fix already exists in the file that needed it

`test_cutlass_mla_decode.py`: **all 49 tests fail on sm_120; all 49 skip on
sm_86** (as intended -- CUTLASS MLA decode needs true SM100 hardware, per the
runbook's tensor-memory/UMMA argument in §02). The failures are a real kernel
error, not a Python exception:

```
RuntimeError: runMla, csrc/.../sm100_cutlass_mla_kernel.cu:209, Error Internal
```

The file's skip condition is the same bug as yesterday's NVFP4 finding --
`has_device_capability(100)`, numeric, true on sm_120 -- but this time it is
caught with the fix sitting three lines away, unused for the check that
matters:
...
@pytest.mark.skipif(
    not current_platform.has_device_capability(100),   # <- wrong function
    reason=CUTLASS_MLA_UNSUPPORTED_REASON,
)
```

`is_device_capability_family()` exists in vLLM's own platform interface --
`(current_capability.to_int() // 10) == (capability // 10)`, the correct family
comparison -- and is used *only* to word the skip reason, not to decide whether
to skip. Confirmed directly on the laptop: `has_device_capability(100)` is
`True` and `is_device_capability_family(100)` is `False` for the same sm_120
device. The reason string this test would print if it skipped correctly --
"Cutlass MLA Requires compute capability of 100 or above" -- describes exactly
the mistake being made.

**Searched the rest of the suite for the same pattern (21 files gate on
`has_device_capability(100)`).** Only one other file, `test_flashinfer_mla_decode.py`,
shares the identical module-level shape and wrong-reason-string language. It
does not fail, though -- 9/9 pass on sm_120 -- because FlashInfer's MLA kernel
happens to actually run on consumer Blackwell where CUTLASS's does not, so the
loose gate is harmless there. `test_flashinfer_nvfp4_scaled_mm.py` uses *both*
functions correctly, gating its SM100-only cute-dsl backend on the family check
while gating its SM120-specific b12x backend on the numeric one -- proof the
codebase knows the distinction in the one place it was applied carefully.

This is the strongest upstream-contribution candidate the lab has produced: an
exact file, an exact line, a fix that is already written elsewhere in the same
file, and independent confirmation that applying it elsewhere in the same
codebase does not break anything.
