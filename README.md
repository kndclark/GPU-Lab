...
Three things the build taught that the plan did not know. The source build against
...
kernels, not to *differentiate* them. QLoRA does not inject facts — the pipeline
works, both adapters confabulated, and the right tool for "the model should know
my lab" is retrieval. And the two cards are worth more fused than the plan
assumed: pooling was scoped as a curiosity and turned out to be the only route to
a 70B-class model here, which is now the lab's most-used capability that neither
node can provide alone.

Phase 3 has started. [kernels/sm-differ](kernels/sm-differ/) is the regression
differ — Rust, taking its measurements from Prometheus rather than parsing logs —
and its tests run inside the push gate alongside the config checks.
